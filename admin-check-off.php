...
#wpfooter, #ninja_forms_selector {
  ...
  global $current_user;
      ...
      if($super_user_status!== 'yes') {
        ...
        <!-- HIDE EVERYTHING -->
        <style>
        #wp-admin-bar-top-secondary, #wp-admin-bar-wp-logo, #wp-admin-bar-comments, #wp-admin-bar-new-content, #adminmenu > li, #welcome-panel, #dashboard-widgets-wrap, #wp-admin-bar-updates {
          display: none !important;
        }

        #adminmenu > li#toplevel_page_ninja-forms, #adminmenu > li#menu-posts-press, #adminmenu > li#menu-posts {
